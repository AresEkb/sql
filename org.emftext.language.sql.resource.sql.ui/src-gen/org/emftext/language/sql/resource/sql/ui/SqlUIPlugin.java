 ...
 */
public class SqlUIPlugin extends AbstractUIPlugin {
	
	public static final String PLUGIN_ID = "org.emftext.language.sql.resource.sql.ui";
	public static final String EDITOR_ID = "org.emftext.language.sql.resource.sql.ui.SqlEditor";
	...
	private static SqlUIPlugin plugin;
	...
	public static SqlUIPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, SqlUIPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, SqlUIPlugin.PLUGIN_ID, message);
		}
		final SqlUIPlugin pluginInstance = SqlUIPlugin.getDefault();
