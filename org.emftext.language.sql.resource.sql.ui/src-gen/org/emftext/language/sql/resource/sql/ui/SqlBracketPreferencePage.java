 ...
 * </p>
 */
public class SqlBracketPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new org.emftext.language.sql.resource.sql.mopp.SqlMetaInformation().getSyntaxName();
	
	private org.emftext.language.sql.resource.sql.ui.SqlBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public SqlBracketPreferencePage() {
		super();
		
		org.emftext.language.sql.resource.sql.ISqlMetaInformation metaInformation = new org.emftext.language.sql.resource.sql.mopp.SqlMetaInformation();
	...
	public void init(IWorkbench workbench) {
		setPreferenceStore(org.emftext.language.sql.resource.sql.ui.SqlUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new org.emftext.language.sql.resource.sql.ui.SqlBracketSet();
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.deserialize(getPreferenceStore().getString(language + org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
	...
	private void addListenersToStyleButtons() {
		...
		bracketsList.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				org.emftext.language.sql.resource.sql.ISqlBracketPair bracketPair = getSelectedBracketPair();
				if (bracketPair == null) {
					removeBracketButton.setEnabled(false);
					return;
				}
	...
	protected void performDefaults() {
		...
		enableCheckbox.setSelection(preferenceStore.getDefaultBoolean(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		matchingBracketsColorEditor.setColorValue(PreferenceConverter.getDefaultColor(preferenceStore, BRACKETS_COLOR));
		String defaultBrackets = preferenceStore.getDefaultString(language + org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	private void updateActiveEditor() {
		...
		preferenceStore.setValue(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		preferenceStore.setValue(language + org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		IWorkbench workbench = org.eclipse.ui.PlatformUI.getWorkbench();
		IEditorPart editor = workbench.getActiveWorkbenchWindow().getActivePage().getActiveEditor();
		if (editor != null && editor instanceof org.emftext.language.sql.resource.sql.ui.SqlEditor) {
			((org.emftext.language.sql.resource.sql.ui.SqlEditor) editor).invalidateTextRepresentation();
		}
	}
	
	protected org.emftext.language.sql.resource.sql.ISqlBracketPair getSelectedBracketPair() {
