 ...
 */
public class SqlContentAssistPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	public SqlContentAssistPreferencePage() {
		initialize();
	}
	...
	public void init(IWorkbench workbench) {
		...
		// init values
		assistEnabled = preferenceStore.getBoolean(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_ENABLED);
		activationDelay = preferenceStore.getInt(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_DELAY);
		activationTriggers = preferenceStore.getString(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_TRIGGERS);
	}
	...
	protected void performDefaults() {
		IPreferenceStore preferenceStore = getPreferenceStore();
		assistEnabled = preferenceStore.getDefaultBoolean(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_ENABLED);
		activationDelay = preferenceStore.getDefaultInt(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_DELAY);
		activationTriggers = preferenceStore.getDefaultString(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_TRIGGERS);
		// defaults in widgets
	...
	private boolean updatePreferences() {
		...
		try {
			...
			org.emftext.language.sql.resource.sql.mopp.SqlPlugin.getDefault().getLog().log(			new Status(IStatus.ERROR			, org.emftext.language.sql.resource.sql.ui.SqlUIPlugin.getDefault().getBundle().getSymbolicName()			, "Value '" + delayText + "' is no valid delay value"			, exception)			);
			return false;
		}
		activationTriggers = txtTriggers.getText();
		IPreferenceStore preferenceStore = getPreferenceStore();
		preferenceStore.setValue(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_ENABLED, assistEnabled);
		preferenceStore.setValue(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_DELAY, activationDelay);
		preferenceStore.setValue(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_CONTENT_ASSIST_TRIGGERS, activationTriggers);
		return true;
	}
