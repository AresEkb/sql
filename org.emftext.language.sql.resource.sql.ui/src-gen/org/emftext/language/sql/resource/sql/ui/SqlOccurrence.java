...
public class SqlOccurrence {
	...
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.sql.resource.sql.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.sql.resource.sql.ui.occurences.declaration";
	
	private final static org.emftext.language.sql.resource.sql.ui.SqlPositionHelper positionHelper = new org.emftext.language.sql.resource.sql.ui.SqlPositionHelper();
	 ...
	 */
	private org.emftext.language.sql.resource.sql.ISqlTextResource textResource;
	...
	/**
	 * <p>
	 * Creates a SqlOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public SqlOccurrence(org.emftext.language.sql.resource.sql.ISqlTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.emftext.language.sql.resource.sql.ISqlLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.emftext.language.sql.resource.sql.analysis.SqlDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.emftext.language.sql.resource.sql.ISqlLocationMap locationMap = textResource.getLocationMap();
		
		org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.emftext.language.sql.resource.sql.ui.ISqlTokenScanner createTokenScanner() {
		return new org.emftext.language.sql.resource.sql.ui.SqlUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.emftext.language.sql.resource.sql.ui.SqlOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.sql.resource.sql.ui.SqlOccurrence.DECLARATION_ANNOTATION_ID);
	}
