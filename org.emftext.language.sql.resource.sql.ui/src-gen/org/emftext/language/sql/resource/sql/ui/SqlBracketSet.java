...
public class SqlBracketSet {
	...
	private ArrayList<org.emftext.language.sql.resource.sql.ISqlBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public SqlBracketSet() {
		super();
		this.languageID = new org.emftext.language.sql.resource.sql.mopp.SqlMetaInformation().getSyntaxName();
		this.bracketPairs = new ArrayList<org.emftext.language.sql.resource.sql.ISqlBracketPair>();
	}
	...
	public org.emftext.language.sql.resource.sql.ISqlBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public org.emftext.language.sql.resource.sql.ISqlBracketPair getBracketPair(String opening, String closing) {
		for (org.emftext.language.sql.resource.sql.ISqlBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new org.emftext.language.sql.resource.sql.mopp.SqlBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public org.emftext.language.sql.resource.sql.ISqlBracketPair remove(String opening, String closing) {
		for (org.emftext.language.sql.resource.sql.ISqlBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<org.emftext.language.sql.resource.sql.ISqlBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
