...
public class SqlHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public SqlHyperlinkDetector(Resource resource) {
		textResource = (org.emftext.language.sql.resource.sql.ISqlTextResource) resource;
	}
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new org.emftext.language.sql.resource.sql.ui.SqlHyperlink(new Region(offset, length), resolvedEObject, text);
