 ...
 */
public class SqlHighlighting implements ISelectionProvider, ISelectionChangedListener {
	...
	private org.emftext.language.sql.resource.sql.ui.SqlEditor editor;
	private ProjectionViewer projectionViewer;
	private org.emftext.language.sql.resource.sql.ui.SqlOccurrence occurrence;
	private org.emftext.language.sql.resource.sql.ui.SqlBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements KeyListener, VerifyListener, MouseListener, org.emftext.language.sql.resource.sql.ISqlBackgroundParsingListener {
	 ...
	 */
	public SqlHighlighting(org.emftext.language.sql.resource.sql.ISqlTextResource textResource, ProjectionViewer projectionViewer, org.emftext.language.sql.resource.sql.ui.SqlColorManager colorManager, org.emftext.language.sql.resource.sql.ui.SqlEditor editor) {
		this.display = Display.getCurrent();
		projectionViewer.getSelectionProvider();
		this.preferenceStore = org.emftext.language.sql.resource.sql.ui.SqlUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		this.textWidget = projectionViewer.getTextWidget();
		this.projectionViewer = projectionViewer;
		this.occurrence = new org.emftext.language.sql.resource.sql.ui.SqlOccurrence(textResource, projectionViewer);
		this.bracketSet = new org.emftext.language.sql.resource.sql.ui.SqlBracketSet();
		this.colorManager = colorManager;
		this.isHighlightBrackets = preferenceStore.getBoolean(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		this.bracketColor = colorManager.getColor(PreferenceConverter.getColor(preferenceStore, org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		this.black = colorManager.getColor(new RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(org.emftext.language.sql.resource.sql.ui.SqlEditor editor) {
	...
	private void setBracketHighlighting(IDocument document) {
		...
		Position[] positions = positionHelper.getPositions(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(IDocument document, String category) {
		...
		if (category.equals(org.emftext.language.sql.resource.sql.ui.SqlPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new org.emftext.language.sql.resource.sql.ui.SqlEObjectSelection(selectedEObject));
				}
			}
		});
	}
	...
	public void resetValues() {
		isHighlightBrackets = preferenceStore.getBoolean(org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(PreferenceConverter.getColor(preferenceStore, org.emftext.language.sql.resource.sql.ui.SqlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
