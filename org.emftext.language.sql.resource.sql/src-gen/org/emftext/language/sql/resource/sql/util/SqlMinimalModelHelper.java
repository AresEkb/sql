 ...
 */
public class SqlMinimalModelHelper {
	
	private final static org.emftext.language.sql.resource.sql.util.SqlEClassUtil eClassUtil = new org.emftext.language.sql.resource.sql.util.SqlEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.emftext.language.sql.resource.sql.util.SqlStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.emftext.language.sql.resource.sql.util.SqlListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.emftext.language.sql.resource.sql.util.SqlStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.emftext.language.sql.resource.sql.util.SqlListUtil.castListUnchecked(value);
