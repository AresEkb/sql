...
/**
 * The SqlTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * SqlTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class SqlTaskItemDetector {
	...
	public List<org.emftext.language.sql.resource.sql.mopp.SqlTaskItem> findTaskItems(String text, int line, int charStart) {
		List<org.emftext.language.sql.resource.sql.mopp.SqlTaskItem> foundItems = new ArrayList<org.emftext.language.sql.resource.sql.mopp.SqlTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.sql.resource.sql.mopp.SqlTaskItem(keyword, message, localLine, offset, end));
