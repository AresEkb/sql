...
public class SqlSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public SqlSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.emftext.language.sql.resource.sql.mopp.SqlLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.emftext.language.sql.resource.sql.mopp.SqlLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.emftext.language.sql.resource.sql.mopp.SqlLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.emftext.language.sql.resource.sql.mopp.SqlLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.emftext.language.sql.resource.sql.mopp.SqlLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = org.emftext.language.sql.resource.sql.util.SqlStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
