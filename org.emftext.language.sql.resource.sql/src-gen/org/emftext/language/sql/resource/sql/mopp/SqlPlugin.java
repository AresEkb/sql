...
public class SqlPlugin extends Plugin {
	...
	public static final String PLUGIN_ID = "org.emftext.language.sql.resource.sql";
	/**
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, SqlPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, SqlPlugin.PLUGIN_ID, message);
		}
		final SqlPlugin pluginInstance = SqlPlugin.getDefault();
