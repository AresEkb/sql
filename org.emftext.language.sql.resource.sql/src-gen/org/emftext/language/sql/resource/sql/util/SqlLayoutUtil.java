...
public class SqlLayoutUtil {
	...
	public void transferLayoutInformationToModel(EObject element) {
		...
		for (Iterator<org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(EObject element) {
		...
		if (layoutReference != null) {
			...
			for (Iterator<EObject> i = list.iterator(); i.hasNext(); ) {
				...
				org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public EObject createLayoutInformationModelElement(org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation layoutInformation, EPackage layoutPackage) {
		EFactory factory = layoutPackage.getEFactoryInstance();
		Object object = layoutInformation.getObject(null, false);
		org.emftext.language.sql.resource.sql.grammar.SqlSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), org.emftext.language.sql.resource.sql.grammar.SqlGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation createLayoutInformation(EObject layoutInformationModelElement) {
		Object object = null;
		...
		org.emftext.language.sql.resource.sql.grammar.SqlSyntaxElement syntaxElement = org.emftext.language.sql.resource.sql.grammar.SqlGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof org.emftext.language.sql.resource.sql.grammar.SqlPlaceholder) {
			org.emftext.language.sql.resource.sql.grammar.SqlPlaceholder placeholder = (org.emftext.language.sql.resource.sql.grammar.SqlPlaceholder) syntaxElement;
			org.emftext.language.sql.resource.sql.ISqlTokenResolver tokenResolver = new org.emftext.language.sql.resource.sql.mopp.SqlTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			org.emftext.language.sql.resource.sql.ISqlTokenResolveResult result = new org.emftext.language.sql.resource.sql.mopp.SqlTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformationAdapter getLayoutInformationAdapter(EObject element) {
	...
	public void removeLayoutInformationAdapter(EObject element) {
		org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformationAdapter existingAdapter = getLayoutInformationAdapter(element);
