...
public class SqlDefaultNameProvider implements org.emftext.language.sql.resource.sql.ISqlNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.sql.resource.sql.util.SqlEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
