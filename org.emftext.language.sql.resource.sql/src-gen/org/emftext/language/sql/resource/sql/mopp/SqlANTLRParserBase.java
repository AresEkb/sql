...
public abstract class SqlANTLRParserBase extends Parser implements org.emftext.language.sql.resource.sql.ISqlTextParser {
	 ...
	 * result object.
	 */
	protected Collection<org.emftext.language.sql.resource.sql.ISqlCommand<org.emftext.language.sql.resource.sql.ISqlTextResource>> postParseCommands;
	 ...
	 */
	private org.emftext.language.sql.resource.sql.mopp.SqlTokenResolveResult tokenResolveResult = new org.emftext.language.sql.resource.sql.mopp.SqlTokenResolveResult();
	...
	public SqlANTLRParserBase(TokenStream input) {
		super(input);
	}
	
	public SqlANTLRParserBase(TokenStream input, RecognizerSharedState state) {
		super(input, state);
	}
	
	protected void retrieveLayoutInformation(EObject element, org.emftext.language.sql.resource.sql.grammar.SqlSyntaxElement syntaxElement, Object object, boolean ignoreTokensAfterLastVisibleToken) {
		...
		isElementToStore |= syntaxElement instanceof org.emftext.language.sql.resource.sql.grammar.SqlPlaceholder;
		isElementToStore |= syntaxElement instanceof org.emftext.language.sql.resource.sql.grammar.SqlKeyword;
		isElementToStore |= syntaxElement instanceof org.emftext.language.sql.resource.sql.grammar.SqlEnumerationTerminal;
		isElementToStore |= syntaxElement instanceof org.emftext.language.sql.resource.sql.grammar.SqlBooleanTerminal;
		if (!isElementToStore) {
			return;
		}
		org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformationAdapter layoutInformationAdapter = getLayoutInformationAdapter(element);
		...
		layoutInformationAdapter.addLayoutInformation(new org.emftext.language.sql.resource.sql.mopp.SqlLayoutInformation(syntaxElement, object, offset, hiddenTokenText.toString(), visibleTokenText.toString()));
	...
	protected <ContainerType extends EObject, ReferenceType extends EObject> void registerContextDependentProxy(final org.emftext.language.sql.resource.sql.mopp.SqlContextDependentURIFragmentFactory<ContainerType, ReferenceType> factory, final ContainerType container, final EReference reference, final String id, final EObject proxy) {
		final int position;
		if (reference.isMany()) {
	...
	protected void addMapEntry(EObject element, EStructuralFeature structuralFeature, org.emftext.language.sql.resource.sql.mopp.SqlDummyEObject dummy) {
		Object value = element.eGet(structuralFeature);
		Object mapKey = dummy.getValueByName("key");
		Object mapValue = dummy.getValueByName("value");
		if (value instanceof EMap<?, ?>) {
			EMap<Object, Object> valueMap = org.emftext.language.sql.resource.sql.util.SqlMapUtil.castToEMap(value);
	...
	protected EObject apply(EObject target, List<EObject> dummyEObjects) {
		...
		for (EObject object : dummyEObjects) {
			assert(object instanceof org.emftext.language.sql.resource.sql.mopp.SqlDummyEObject);
			org.emftext.language.sql.resource.sql.mopp.SqlDummyEObject dummy = (org.emftext.language.sql.resource.sql.mopp.SqlDummyEObject) object;
	...
	protected org.emftext.language.sql.resource.sql.mopp.SqlTokenResolveResult getFreshTokenResolveResult() {
		tokenResolveResult.clear();
		return tokenResolveResult;
	}
	
	protected org.emftext.language.sql.resource.sql.mopp.SqlReferenceResolverSwitch getReferenceResolverSwitch() {
		org.emftext.language.sql.resource.sql.mopp.SqlReferenceResolverSwitch resolverSwitch = (org.emftext.language.sql.resource.sql.mopp.SqlReferenceResolverSwitch) metaInformation.getReferenceResolverSwitch();
	...
	public boolean isLayoutInformationRecordingEnabled() {
		...
		Object value = options.get(org.emftext.language.sql.resource.sql.ISqlOptions.DISABLE_LAYOUT_INFORMATION_RECORDING);
	...
	public boolean isLocationMapEnabled() {
		...
		Object value = options.get(org.emftext.language.sql.resource.sql.ISqlOptions.DISABLE_LOCATION_MAP);
		return value == null || Boolean.FALSE.equals(value);
	}
